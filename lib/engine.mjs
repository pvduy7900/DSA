...
export function isClassProblem(problem) {
  return problem.tests.some((t) => t.type === "class");
}

export function getDefaultRunInput(problem) {
  if (isClassProblem(problem)) return "";
  const first = problem.tests.find((t) => t.type !== "class");
  if (!first?.args) return "";
  if (first.args.length === 1 && typeof first.args[0] === "string") return first.args[0];
  return JSON.stringify(first.args);
}

export function parseRunInput(raw) {
  const trimmed = raw.trim();
  if (!trimmed) throw new Error("Nhập input để chạy");
  try {
...
async function runFunctionOnce(problem, module, args) {
  const capture = createConsoleCapture();
  try {
    const fn = module[problem.function];
    if (typeof fn !== "function") {
      throw new Error(`Function "${problem.function}" not found`);
    }
    const returnValue = fn(...args);
    return { consoleOutput: capture.lines, returnValue, error: null };
  } catch (error) {
    return { consoleOutput: capture.lines, returnValue: null, error: error.message };
  } finally {
    capture.restore();
  }
}

async function runClassOnce(problem, module) {
  const test = problem.tests.find((t) => t.type === "class");
  const capture = createConsoleCapture();
  try {
    if (!test) throw new Error("No class test found");
    const Cls = module[problem.function];
    if (typeof Cls !== "function") {
      throw new Error(`Class "${problem.function}" not found`);
    }
    const instance = new Cls();
    for (const op of test.operations) {
      instance[op.op](...(op.args || []));
    }
    return { consoleOutput: capture.lines, returnValue: null, error: null };
  } catch (error) {
    return { consoleOutput: capture.lines, returnValue: null, error: error.message };
  } finally {
    capture.restore();
...
async function withUserModule(problem, sourceCode, root, runner) {
  ...
  try {
    ...
    return await runner(module);
  } finally {
    rmSync(tempDir, { recursive: true, force: true });
  }
}

export async function runSourceOnce(problem, sourceCode, input, root = join(ROOT, "..")) {
  return withUserModule(problem, sourceCode, root, async (module) => {
    if (isClassProblem(problem)) {
      return runClassOnce(problem, module);
    }
    const args = parseRunInput(input);
    return runFunctionOnce(problem, module, args);
  });
}
